            ...
            <CarouselContent className='w-[99%] pl-6'>
                {Array.from({ length: 5 }).map((_, index) => (
                    <CarouselItem key={index} className=" mx-3 min-h-[100px] py-10 rounded-lg bg-secondary ">
                        ...
                        <CarouselPrevious className="absolute left-0 md:top-37 sm:top-40 xs:top-40 top-35 " />
                        <CarouselNext className="absolute md:top-37 sm:top-40 xs:top-40 top-35  right-5" />
                        </div>
                    </CarouselItem>
                ))}
            </CarouselContent>
